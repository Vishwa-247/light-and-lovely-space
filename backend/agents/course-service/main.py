...
class CourseCreateRequest(BaseModel):
    ...
    purpose: str = Field(..., regex="^(exam|job_interview|practice|coding_preparation|other)$")
    difficulty: str = Field(..., regex="^(beginner|intermediate|advanced|expert)$")
    summary: Optional[str] = Field(None, max_length=1000)
    user_id: str = Field(..., description="User UUID")

class CourseUpdateRequest(BaseModel):
    title: Optional[str] = Field(None, min_length=1, max_length=200)
    purpose: Optional[str] = Field(None, regex="^(exam|job_interview|practice|coding_preparation|other)$")
    difficulty: Optional[str] = Field(None, regex="^(beginner|intermediate|advanced|expert)$")
    summary: Optional[str] = Field(None, max_length=1000)
    status: Optional[str] = Field(None, regex="^(draft|generating|ready|published)$")
...
class ProgressTrackRequest(BaseModel):
    ...
    progress_type: str = Field(..., regex="^(chapter_read|flashcard_reviewed|mcq_answered|resource_viewed)$")
    score: Optional[int] = Field(None, ge=0, le=100)
    time_spent: Optional[int] = Field(None, ge=0)

class AIContentGenerationRequest(BaseModel):
    course_id: str
    content_type: str = Field(..., regex="^(flashcards|mcqs|qnas|notebook|resources)$")
    topic: str = Field(..., min_length=1)
    difficulty: Optional[str] = Field("medium", regex="^(easy|medium|hard)$")
...
async def health_check():
    """Health check endpoint."""
    try:
        db_status = await asyncio.to_thread(db_health_check) if supabase_manager else {"status": "offline"}
        return {
            "status": "healthy",
